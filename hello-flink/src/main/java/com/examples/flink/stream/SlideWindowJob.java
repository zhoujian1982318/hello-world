...
public class SlideWindowJob {
	private static ObjectMapper mapper = new ObjectMapper();

	private static SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

	private static Logger LOG = LoggerFactory.getLogger(SlideWindowJob.class);
	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<DeviceAggr> aggrDs = detailDs.assignTimestampsAndWatermarks(new DeviceTimeStamp())
				...
				.timeWindow(Time.hours(2),Time.hours(1))
				.aggregate(aggreFun, processWindowFunction)
				.name("device-aggreate-stream");
		
		aggrDs.writeAsText("devicOutputSlideWindow.txt",WriteMode.OVERWRITE).name("fileSink").setParallelism(1);
